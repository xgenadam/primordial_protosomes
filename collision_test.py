...
while not done:
    ...
    array = petri_dish.draw_to_array(surface_array, screen_center, ppm=1, direction=None)
    pygame.surfarray.blit_array(screen, array)

    petri_dish.update_world(timestep=TIME_STEP)

    collisions = petri_dish.collisions

    kill_list = chain(collisions.get(test_creature_5.uuid, []), collisions.get(petri_dish.walls.uuid, []))

    if kill_list:
        petri_dish.destroy_physicals(kill_list)
